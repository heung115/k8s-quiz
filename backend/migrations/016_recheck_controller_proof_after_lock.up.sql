-- Forward-only security fix: validation time must be sampled after every
-- potentially blocking authority lookup. Otherwise a proof that was valid on
-- function entry could wait behind the epoch row lock and be accepted after
-- both its proof expiry and effect deadline.
CREATE OR REPLACE FUNCTION public.consume_runner_controller_proof(
    p_provider_id TEXT,
    p_epoch BIGINT,
    p_lease_id UUID,
    p_proof_id UUID,
...
BEGIN
    ...
    -- Sample only after both the authority-row wait and proof-row
    -- serialization. This is the time at which the effect is actually
    -- authorized, not function entry.
...
REVOKE ALL ON FUNCTION public.consume_runner_controller_proof(
