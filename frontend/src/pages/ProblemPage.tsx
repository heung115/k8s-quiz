...
export function ProblemPage() {
  ...
  const handleEnd = async () => {
    try { await api.delete('/api/sessions/current') } catch { /* ignore */ }
    clear(); navigate('/')
  }
          ...
          <div className="p-5 space-y-5">
            ...
            {isChoice && session && problem.choices && (
              <div className="border border-edge bg-surface p-4">
                ...
                <div className="space-y-2" role="radiogroup" aria-label="원인 선택">
                  {problem.choices.map((choice) => (
                    <label key={choice.id} className={`flex items-start gap-3 border p-3 cursor-pointer transition-colors text-sm focus-within:border-accent/70 focus-within:bg-accent-dim ${selectedChoice === choice.id ? 'border-accent/60 bg-accent-dim text-ink' : 'border-edge text-ink-muted hover:bg-surface-2'}`}>
                      <input type="radio" name="choice" value={choice.id} checked={selectedChoice === choice.id} onChange={() => setSelectedChoice(choice.id)} className="sr-only" />
                ...
                <button onClick={handleSubmitChoice} disabled={!selectedChoice || verifying} className="btn-primary w-full mt-3 text-sm py-2.5 disabled:opacity-40 disabled:shadow-none disabled:hover:translate-y-0">
            ...
            {session && (
              <div className="flex gap-2 pt-1">
                {(problem.verify_type === 'script' || problem.verify_type === 'text') && (
                  <button onClick={handleVerify} disabled={verifying || crashed || status !== 'ready'} className="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 border border-success/40 bg-success-soft text-success hover:bg-success/20 disabled:opacity-40 text-sm font-display font-semibold tracking-wide transition-colors">
                    <CheckCircle2 className="w-4 h-4" aria-hidden="true" />{verifying ? '검증 중…' : '검증'}
                  </button>
                )}
                <button onClick={handleReset} className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 border border-edge bg-surface-2 hover:bg-surface-3 text-sm transition-colors"><RotateCcw className="w-3.5 h-3.5" aria-hidden="true" /> 리셋</button>
                <button onClick={handleEnd} className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 border border-edge bg-surface-2 hover:border-danger/40 hover:bg-danger-soft hover:text-danger text-sm transition-colors"><Square className="w-3.5 h-3.5" aria-hidden="true" /> 종료</button>
              </div>
            )}
